...
# tibble 패키지 rownames_to_column()를 이용하여 data( 행 )를 변수( 열 )로 변경
...
#
# Data Mining : 대규모로 저장된 데이터 안에서 체계적이고 자동적으로 통계적 규칙이나 패턴을 찾아내는 것을 말하며
#               KDD( Knowledge- discovery in databases, 데이터베이스 속의 지식 발견 )
#
# Text Mining : 비정형 데이터 마이닝의 유형 중 하나 
#               비정형 / 반정형 데이터에 대하여 자연어 처리 기술과 문서 처리 기술을 적용하여 유용한 정보를 추출,
#               가공하는 목적으로 하는 기술
#
# Word Cloud : 텍스트 데이터를 분석하는 대표 기술
# ( BoW )      대상 데이터에서 단어( 주로 명사 )를 추출하고 단어들의 출현 빈도수를 계산하여 시각화하는 도구 
#  ㄴ Bag of   출현 빈도수가 높은 단어는 그만큼 중요하거나 관심도가 높다는 것을 의미 
...
# 8. 한글 Word Cloud 표현
#
# R에서 한글 WordCloud를 이용하기 위해서는 Java 실행 환경( JRE ) 필요
...
# 4, 자료 수집( Text 자료 )
setwd( "C:\\Workspace\\WorkspaceR\\wordcloud" )
...
noun2 <- noun2[ nchar( noun2 ) > 1 ]  # 빈도수가 2 이상
noun2 <- gsub( '하지', '', noun2 )
noun2 <- gsub( '때문', '', noun2 )
wordcount <- table( noun2 )
