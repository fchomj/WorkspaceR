...
# : 데이터의 분산( Variance )를 최대한 보존하면서 서로 직교하는 축을 찾아 고차원 공간의 표본을
#   저 차원으로 변환하는 차원 축소 기법
#
...
dup <- which( duplicated( ds ) )    # 중복 데이터 확인 - 위치 o ( 번째 ) / 수 x
dup
...
tsne <- Rtsne( ds, dims = 2, perplexity = 10 )    # Rtsne : 차원 축소 함수 
tsne
...
#
# 데이터 과학에서 
#    독립변수( independent variable ), 설명변수( explanatory variable ), 특징( feature ) -> X라 하고
#
#    종속변수( dependent variable ), 반응변수( response variable ), 레이블( label, 정답 ) -> y라 할때
#
#    X가 입력되면 y를 맞추어야 하는 문제, y를 ground truth로 간주
#
#
# 회귀분석( Regression Analysis )
#  : 회귀 이론을 기초로 독립변수 X가 종속변수 y에 미치는 영향을 파악하여 예측 모델을 도출하는 통계적인 방법
#
...
#     x를 통해 y 예측 
# 
# 단순선형 회귀모델의 회귀식( 모델 ) : y = WX + b ( W, b는 상수 )
#   X : 독립변수
#   y : 종속변수
#
...
model <- lm( dist~speed, cars )    # 회귀 모델 만들 때 사용하는 함수 
model
...
#     이때 귀무가설은 용의자는 무죄라는 가설
#
# t-통계량( t-statistics ) 또는 t-값( t-value )
#  : 데이터의 크기는 평균은 클수록, 분산은 작을수록, 데이터 크기가 클수록 믿음이 커진다에서 이 세 가지 값을 묶은 것
#
...
# 이 때 계산된 확률값을 p값이라 함
# p값이 매우 낮으면, 이러한 표본 통계값은 우연히 나타나기 어려운 케이스이기 때문에, 우리는 귀무가설을 채택하지
# 않고( 기각하고 ), 대안적인 가설인 대립가설을 채택한다.
#
summary( model )
